...
Cname: ID {
	...
	|ID EXTENDS ID {
		...
		CInstall($<Node>1->varname,$<Node>3->varname);
		//copy all the memeber fields of parent class to the child.
		Class= Clookup($<Node>3->varname);
		Ftemp= Class->Memberfield;
		while(Ftemp!=NULL){
			if(Ftemp->type != NULL){
				Class_Finstall(Clookup($<Node>1->varname),Ftemp->type->name,Ftemp->name);
			}
			else{
				Class_Finstall(Clookup($<Node>1->varname),Ftemp->Ctype->name, Ftemp->name);
			}
			Ftemp=Ftemp->next;
		}
		...
		while(Mtemp!=NULL){
			Class_Minstall(Clookup($<Node>1->varname), Mtemp->name, Mtemp->type, Mtemp->paramlist);
			Mtemp1=Class_Mlookup(Clookup($<Node>1->varname),Mtemp->name);
...
Field: ID '.' ID 	{
	if(Class!=NULL){
		printf("private memeber %s access not allowed",$<Node>3->varname);
		exit(0);
	}
	...
	| Field '.' ID	{
		if(strcmp($<Node>1->varname,"\0")!=0 && $<Node>1->type==NULL){
			printf("private members %s access not allowd",$<Node>3->varname);
			exit(0);
		}
		struct tnode *t1, *t2;
		t2= $<Node>1;
...
;
FieldFunction: ID '.' ID '(' ArgList ')' { //occur outside class.
